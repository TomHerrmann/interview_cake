...
const cafeOrderChecker = (takeOutOrders, dineInOrders, servedOrders) => {
  ...
  for (let servedIndex = 0; servedIndex < servedOrders.length; servedIndex++) {
    if (servedOrders[servedIndex] === takeOutOrders[takeOutIndex]) {
      takeOutIndex++;
    } else if (servedOrders[servedIndex] === dineInOrders[dineInIndex]) {
      dineInIndex++;
    } else return false;
  }
...
console.log(cafeOrderChecker([1, 3, 5], [2, 4, 6], [1, 2, 4, 6, 5, 3])); // => false
console.log(cafeOrderChecker([17, 8, 24], [12, 19, 2], [17, 8, 12, 19, 24, 2])); // => true
